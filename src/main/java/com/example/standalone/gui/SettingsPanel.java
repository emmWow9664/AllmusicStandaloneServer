...
public class SettingsPanel extends JPanel {
    ...
    private void buildItems() {
        ...
        addItem(new Item("standalone.webEnabled", "Web 面板", "webEnabled",
                "是否启用内嵌 Web 展示与管理面板（修改后需重启服务端）", "bool"));
        addItem(new Item("standalone.webPort", "Web 端口", "webPort",
                "内嵌 Web 面板监听端口，默认 8080（修改后需重启服务端）", "int"));
        addItem(new Item("standalone.webBindHost", "Web 绑定地址", "webBindHost",
                "Web 面板绑定地址，0.0.0.0 为所有网卡（修改后需重启服务端）", "str"));
        addWebPasswordItem();

        addGroup("关于");
        listPanel.add(info("AllmusicStandaloneServer", "AllMusic 独立音乐服务器（Standalone Server）"));
        listPanel.add(info("版本", "1.0.14"));
